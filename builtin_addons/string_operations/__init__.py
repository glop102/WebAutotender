...
async def str_buildWithVars(instance: Instance, inputString:String, outputVarname: VariablePath) -> CommandReturnStatus:
    """Build a string using Python-style {variable_name} placeholders.
  inputString: A format string with {variable_name} placeholders. Instance variables are looked up first, then the workflow constants, then the globals. Secrets are deliberately not available here.
